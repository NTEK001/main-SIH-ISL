...
def translate_and_speak_hindi(sentence):
    if sentence:
        try:
            ...
            # print(f"Translated Sentence: {translated_text}")  # Optional: print the translated sentence

            # Speak the translated sentence in Hindi
            tts = gTTS(translated_text, lang='hi')
            tts.save("translated_sentence.mp3")
            playsound("translated_sentence.mp3")

            # Clean up the mp3 file after playing
            os.remove("translated_sentence.mp3")

        except Exception as e:
            print(f"Error occurred during translation or speaking: {e}")


def translate_and_speak_tamil(sentence):
    if sentence:
        try:
            # Initialize the translator
            translator = Translator()
            ...
            # Translate the sentence from English to Hindi
            translated = translator.translate(sentence, src='en', dest='ta')
            translated_text = translated.text

            # print(f"Translated Sentence: {translated_text}")  # Optional: print the translated sentence

            # Speak the translated sentence in Tamil
...
class Camera_widget(QWidget, Ui_Form):
    def __init__(self):
        ...
        # Connect existing buttons to their functions
        self.pushButton.clicked.connect(self.Worker2.speekbuttonclicked)

        # Connect the translate button to the translate_and_speak function
        self.pushButton_2.clicked.connect(lambda: translate_and_speak_hindi(sentence))

        self.pushButton_3.clicked.connect(lambda: translate_and_speak_tamil(sentence))
...
class worker2(QThread):
    ...
    def run(self):
        ...
        classifier = Classifier("./Sign Language Model2 new.keras", "./labels.txt")
        offset = 20
        imgSize = 300
